...
begin
  ...
  -- Força da espécie/forma representada pelo Tier. Apenas E-S normal são capturáveis.
  ...
  -- Evoluções posteriores são progressivamente mais difíceis.
  evo_factor:=case evo_stage when 1 then 1.00 when 2 then .82 else .68 end;

  -- Lv1-10 não recebe penalidade; depois disso a dificuldade cresce gradualmente.
  level_factor:=greatest(.60,1-greatest(0,p_level-10)*.0045);

  -- Qualidade é independente do Tier: quanto maior o multiplicador, mais difícil.
  quality_factor:=1/sqrt(greatest(1,p_rarity_mult));

  -- Formas especiais continuam mais difíceis, além de Tier/qualidade.
  form_factor:=case
  ...
  -- Chance básica: máximo absoluto de 20% antes de HP/Ball/bônus.
  base_chance:=least(20,20*tier_factor*evo_factor*level_factor*quality_factor*form_factor);

  -- Enfraquecer o alvo ajuda até +65% relativo sobre a base/Ball.
