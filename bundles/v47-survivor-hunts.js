(function(){
'use strict';
const PSY_SURVIVOR_BUILD='SURV_CITY_SUSPEND_20260829_V7';
...
function survLoop(ts){
  ...
  for(const fx of s.effects){if(fx.type==='burst'||fx.type==='impact'||fx.type==='cast'||fx.type==='helperAura'||fx.type==='thunderSeal'||fx.type==='waterCast'||fx.type==='windCast'||fx.type==='meteorWarning'||fx.type==='voidOpen')fx.r+=(Number(fx.max||fx.r)-fx.r)*.22;if(fx.type==='particle'||fx.type==='bubble'){fx.x+=Number(fx.vx||0);fx.y+=Number(fx.vy||0);fx.vy=Number(fx.vy||0)+Number(fx.gravity||0);fx.vx*=Number(fx.drag||.96);fx.vy*=Number(fx.drag||.96)}if(fx.type==='sanctuary'&&Number.isFinite(Number(fx.until)))fx.life=Number(s.elapsed||0)<Number(fx.until)?1:0;else fx.life--}s.effects=s.effects.filter(f=>f.life>0);for(const t of s.texts)t.life--;s.texts=s.texts.filter(t=>t.life>0);for(const a of (s.dropAlerts||[]))a.life-=dt;s.dropAlerts=(s.dropAlerts||[]).filter(a=>a.life>0);s.enemies=s.enemies.filter(e=>!e.dead);s.bullets=s.bullets.filter(b=>!b.dead).slice(-Number(s.perf?.bulletCap||260));s.enemyBullets=s.enemyBullets.filter(b=>!b.dead).slice(-Number(s.perf?.enemyBulletCap||220));if(s.texts.length>Number(s.perf?.textCap||100))s.texts.splice(0,s.texts.length-Number(s.perf?.textCap||100));survCollect(s);
