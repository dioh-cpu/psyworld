(function(){
'use strict';
const PSY_SURVIVOR_BUILD='SURV_PERFORMANCE_QA6_20260831';
...
const SURV_ELITE_INTERVAL=90000; // 1m30
const SURV_TOTAL_PHASES=400;
const SURV_BOSS_TIMES=[180000,360000,540000]; // 3, 6 e 9 minutos
const SURV_FINAL_BOSS_TIME=600000; // minuto 10
const SURV_LAST_ELITE_TIME=540000; // 1:30, 3:00, 4:30, 6:00, 7:30 e 9:00
...
function survNextBossMinute(s){const ms=Math.max(0,Number(s.elapsed||0));return ms<180000?3:ms<360000?6:ms<540000?9:10}
...
function survPerfProfile(){
  ...
  return{mobile:coarse,low,vfxQuality:low?.52:coarse?.72:1,fxCap:low?100:coarse?145:220,particleCap:low?4:coarse?7:10,enemyCap:72,bulletCap:low?120:coarse?160:220,enemyBulletCap:low?100:coarse?140:200,textCap:low?48:coarse?70:100,fieldCap:low?58:coarse?78:105,avgDt:16.7,frameCount:0,drawBudgetAt:0};
}
...
function survDrawPsyduck(s,ctx){
  let im=survEnsurePsyFormImages(s);const form=s.psyForm||'normal';if(!im?.complete||!im.naturalWidth){im=s.psyFormImgs?.normal||s.psyImg;if(!im?.complete||!im.naturalWidth)return}const pal=survPsyAttackPalette(s),drawSize=(form==='mega'||form==='mega_shiny'?88:form==='squizo'?78:72),pulse=.5+.5*Math.sin(s.elapsed/125);ctx.globalAlpha=.13+.08*pulse;ctx.fillStyle=pal.shot;ctx.beginPath();ctx.arc(s.x,s.y,58+4*pulse,0,Math.PI*2);ctx.fill();ctx.globalAlpha=1;ctx.strokeStyle=pal.special;ctx.lineWidth=2;ctx.globalAlpha=.30+.15*pulse;ctx.beginPath();ctx.arc(s.x,s.y,48+7*pulse,0,Math.PI*2);ctx.stroke();ctx.globalAlpha=1;ctx.shadowColor=pal.accent;ctx.shadowBlur=13;ctx.save();ctx.translate(s.x,s.y);if(Number(s.psyFacing||1)<0)ctx.scale(-1,1);
...
function survApplyUpgrade(s,u){
  ...
  else if(lv<u.max){s.upgradeLevels[id]=lv+1;if(id==='rate')s.fireRate=Math.max(135,s.fireRate*.92);if(id==='multi')s.multi=Math.max(1,Number(s.multi||1))+1;if(id==='pierce')s.pierce=Math.max(0,Number(s.pierce||0))+1;if(id==='speed')s.speed*=1.06;if(id==='nova')s.nova++;if(id==='vortex')s.vortex++;if(id==='rain')s.rain++;if(id==='regen'){const hpSteps=[0,.03,.05,.07,.10,.15],oldPct=hpSteps[lv]||0,newPct=hpSteps[Math.min(5,lv+1)]||oldPct,oldMax=s.maxHp;s.maxHp*=((1+newPct)/(1+oldPct));s.hp=Math.min(s.maxHp,s.hp+(s.maxHp-oldMax));s.regen+=.0025}}else return false;
  return true
}
...
function survNearestEnemy(s,x,y,exclude,maxRange=Infinity){let best=null,bd2=maxRange===Infinity?Infinity:maxRange*maxRange;for(const e of s.enemies){if(e.dead||e===exclude)continue;const dx=e.x-x,dy=e.y-y,d2=dx*dx+dy*dy;if(d2<=bd2){bd2=d2;best=e}}return best}
function survEnemiesNear(s,x,y,maxRange){const r2=maxRange*maxRange;return s.enemies.filter(e=>{if(e.dead)return false;const dx=e.x-x,dy=e.y-y;return dx*dx+dy*dy<=r2})}
function survBullet(s,o={}){s.bullets.push({x:o.x,y:o.y,vx:o.vx,vy:o.vy,dmg:o.dmg,pierce:o.pierce||0,chain:o.chain||0,chainMul:o.chainMul||.7,crit:!!o.crit,hit:new Set(),helper:!!o.helper,kind:o.kind||'psy',color:o.color||'#67e8f9',radius:o.radius||5,aoe:o.aoe||0,slow:o.slow||0,life:o.life||1800,travel:0,maxRange:Number(o.maxRange||s.attackRange||SURV_ATTACK_RANGE),homing:!!o.homing,target:o.target||null,impactField:o.impactField||null,onHit:o.onHit||'',homeAcc:0})}
function survShoot(s,originX=s.x,originY=s.y,helper=false){
  ...
  for(let k=0;k<count;k++){const t=targets[k%targets.length],a=Math.atan2(t.y-originY,t.x-originX)+(helper?0:(k-(count-1)/2)*.075);survBullet(s,{x:originX,y:originY,vx:Math.cos(a)*8.8,vy:Math.sin(a)*8.8,dmg:(helper?10:18)*s.power,pierce:Math.max(0,Number(s.pierce||0)),chain:0,chainMul:0,crit:Math.random()<s.crit,helper,kind:'psy',color:helper?'#67e8f9':pal.shot,radius:helper?5:7,maxRange:limit})}
}
...
function survAreaDamage(s,x,y,range,dmg,color,exclude=null,slow=0){const r2=range*range;for(const e of s.enemies){if(e.dead||e===exclude)continue;const dx=e.x-x,dy=e.y-y;if(dx*dx+dy*dy<=r2)survDamageEnemy(s,e,dmg,color,slow)}survFxPush(s,{type:'burst',x,y,r:8,max:range,life:22,color})}
function survHitArea(s,x,y,range,dmg,color,slow=0){const r2=range*range;for(const e of s.enemies){if(e.dead)continue;const dx=e.x-x,dy=e.y-y;if(dx*dx+dy*dy<=r2)survDamageEnemy(s,e,dmg,color,slow)}survFxPush(s,{type:'burst',x,y,r:8,max:range,life:22,color})}
...
function survNova(s){W.psyAudio?.sfx?.('psy_nova',{volume:.9,cooldown:500});survPsyAnimTrigger(s,'special',500);const pal=survPsyAttackPalette(s),range=Math.min(s.attackRange,135+22*s.nova),dmg=18*s.power*(1+s.nova*.18);survHitArea(s,s.x,s.y,range,dmg,pal.special);survFxPush(s,{type:'nova',x:s.x,y:s.y,r:20,max:range,life:28,color:pal.special,color2:pal.shot});survFxParticles(s,s.x,s.y,pal.accent,12,4.4,5,25,.02);s.shake=Math.max(Number(s.shake||0),3);if(s.novaMax)setTimeout(()=>{if(W.PSY_CLEAN_SURV===s&&!s.done){survHitArea(s,s.x,s.y,range,12*s.power,pal.accent);survFxPush(s,{type:'nova',x:s.x,y:s.y,r:16,max:range*.92,life:22,color:pal.shot,color2:pal.special,reverse:true})}},180)}
function survVortex(s){if(!s.vortex)return;W.psyAudio?.sfx?.('psy_vortex',{volume:.82,cooldown:650});survPsyAnimTrigger(s,'special',500);const pal=survPsyAttackPalette(s),range=Math.min(s.attackRange,105+s.vortex*18),dmg=(9+s.vortex*4)*s.power;for(const e of s.enemies){if(e.dead)continue;const dx=s.x-e.x,dy=s.y-e.y,d=Math.hypot(dx,dy);if(d<range){survDamageEnemy(s,e,dmg,pal.special);if(s.vortexMax&&d>12&&!e.dead){e.x+=dx/d*14;e.y+=dy/d*14}}}survFxPush(s,{type:'vortex',x:s.x,y:s.y,r:range,life:28,color:pal.special,color2:pal.shot,maxed:!!s.vortexMax});survFxParticles(s,s.x,s.y,pal.accent,8,3.6,4,20,-.01)}
function survPsyRain(s){if(!s.rain)return;W.psyAudio?.sfx?.('psy_rain',{volume:.82,cooldown:650});survPsyAnimTrigger(s,'special',500);const pal=survPsyAttackPalette(s),live=survShuffle(survEnemiesNear(s,s.x,s.y,s.attackRange)).slice(0,Math.min(2+s.rain,7));for(const e of live){const dmg=(14+s.rain*3.5)*s.power;survFxLightning(s,e.x+(Math.random()-.5)*20,e.y-145,e.x,e.y,pal.special,5,14,1.25);survFxPush(s,{type:'psyblade',x:e.x,y:e.y-52,x2:e.x,y2:e.y+10,color:pal.shot,life:15,w:16});survDamageEnemy(s,e,dmg,pal.special);survFxImpact(s,e.x,e.y,pal.accent,30,false);if(s.rainMax&&!e.dead){survDamageEnemy(s,e,dmg*.55,pal.accent);survFxLightning(s,e.x-18,e.y-120,e.x+10,e.y,pal.shot,3,10,1)}}}
...
function survApplyBlisseySanctuary(s,dt){
  const now=Number(s.elapsed||0);for(let i=0;i<(s.helperIds?.length||0);i++){const key=s.helperIds[i];if(key!=='blissey'||!survHelperMaxed(s,key))continue;const h=survHelperState(s,key,i);if(!h.sanctuaryNext||now>=h.sanctuaryNext)survBlisseySanctuary(s,h,now);if(now>=Number(h.sanctuaryUntil||0))continue;const x=Number(h.sanctuaryX||h.x),y=Number(h.sanctuaryY||h.y),r=SURV_SQM*6;
    ...
    if(Math.hypot(s.x-x,s.y-y)<=r){s.blisseyRegenUntil=Math.max(Number(s.blisseyRegenUntil||0),now+SURV_BLISSEY_REGEN_TAIL);if(h.playerShieldSanctuaryId!==h.sanctuaryId){h.playerShieldSanctuaryId=h.sanctuaryId;s.blisseyShield=Math.max(Number(s.blisseyShield||0),s.maxHp*.05);s.blisseyShieldUntil=Math.max(Number(s.blisseyShieldUntil||0),h.sanctuaryUntil+SURV_BLISSEY_REGEN_TAIL)}}
    for(let j=0;j<(s.helperIds?.length||0);j++){const hk=s.helperIds[j],a=survHelperState(s,hk,j);if(Math.hypot(a.x-x,a.y-y)<=r){a.blisseyRegenUntil=Math.max(Number(a.blisseyRegenUntil||0),now+SURV_BLISSEY_REGEN_TAIL);if(a.blisseyShieldSanctuaryId!==h.sanctuaryId){a.blisseyShieldSanctuaryId=h.sanctuaryId;a.blisseyShield=Math.max(Number(a.blisseyShield||0),a.maxHp*.05);a.blisseyShieldUntil=Math.max(Number(a.blisseyShieldUntil||0),h.sanctuaryUntil+SURV_BLISSEY_REGEN_TAIL)}}}
  }
...
function survUpdateFields(s,dt){
  ...
  for(const f of s.fields){if(f.dead)continue;if(f.startDelay>0){f.startDelay-=dt;continue}
    if(f.kind==='meteor'){survHitArea(s,f.x,f.y,f.radius,f.dmg,f.color);survFxPush(s,{type:'meteorStrike',x:f.x,y:f.y,life:f.giant?24:17,color:f.color,giant:!!f.giant,radius:f.radius});const charLite=f.source==='charizard';survFxImpact(s,f.x,f.y,f.color,Math.min(f.radius,f.giant?180:95),!!f.giant,charLite?.18:1);survFxParticles(s,f.x,f.y,'#fde68a',charLite?(f.giant?5:3):(f.giant?18:10),f.giant?7:4.5,f.giant?7:4,f.giant?24:18,.12);f.dead=true;continue}
    ...
    const every=Number(f.tickEvery||300);while(f.tickAcc>=every){f.tickAcc-=every;
      ...
      else if(f.kind==='blackhole'){for(const e of s.enemies){if(e.dead||f.hitSet.has(e))continue;{const dx=e.x-f.x,dy=e.y-f.y;if(dx*dx+dy*dy<=f.radius*f.radius){f.hitSet.add(e);survDamageEnemy(s,e,Number(f.impact||f.dmg),f.color)}}}}
      else if(f.kind==='tornadozone'){for(const e of s.enemies){if(e.dead||f.hitSet.has(e))continue;{const dx=e.x-f.x,dy=e.y-f.y;if(dx*dx+dy*dy<=f.radius*f.radius){f.hitSet.add(e);survDamageEnemy(s,e,f.dmg,f.color)}}}}
    }
...
function survLoop(ts){
  const s=W.PSY_CLEAN_SURV;if(!s||s.done)return;requestAnimationFrame(survLoop);const rawDt=Math.max(0,ts-(s.last||ts)),dt=Math.min(34,rawDt);s.last=ts;if(s.perf){s.perf.avgDt=s.perf.avgDt*.94+Math.min(60,rawDt||16.7)*.06;s.perf.frameCount++;if((s.perf.frameCount%30)===0){let q=s.perf.avgDt>29?.36:s.perf.avgDt>24?.48:s.perf.avgDt>20?.64:s.perf.low?.58:s.perf.mobile?.76:1;if(s.helperIds?.includes('dragonite')||s.helperIds?.includes('charizard'))q=Math.min(q,.78);s.perf.vfxQuality=q}}if(s.paused){if(ts-Number(s._pauseDrawAt||0)>120){s._pauseDrawAt=ts;survDraw(s)}return}
  ...
  if(s.elapsed>=SURV_BOSS_TIMES[0])survSpawnBoss(s,3,false);if(s.elapsed>=SURV_BOSS_TIMES[1])survSpawnBoss(s,6,false);if(s.elapsed>=SURV_BOSS_TIMES[2])survSpawnBoss(s,9,false);if(s.elapsed>=SURV_FINAL_BOSS_TIME&&!s.finalSpawned){s.finalSpawned=true;survSpawnBoss(s,10,true)}
