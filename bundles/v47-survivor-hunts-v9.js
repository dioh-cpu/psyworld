...
(function(){
'use strict';
const PSY_SURVIVOR_BUILD='SURV_WOBBUFFET_METER_PERF_V16_20260901';
...
const SURV_RUN_UPS=[
  ...
  {id:'vortex',name:'Vórtice Mental',kind:'ÁREA',poke:196,desc:'Pulso em área que danifica inimigos próximos',max:5,maxDesc:'MAX: dobra o tamanho; a cada 5s puxa até 10 SQM, causa dano ao puxar e repele com novo dano ao chegar a 3 SQM'},
  {id:'rain',name:'Chuva Psíquica',kind:'ÁREA',poke:151,desc:'Golpeia alvos aleatórios próximos periodicamente',max:5,maxDesc:'MAX: cada alvo recebe uma segunda descarga e tem 5% de chance de atordoar por 1,5s'},
...
function survRunRequirementLv(s,id){
  const raw=survRunUpLv(s,id),stored=Math.max(0,Number(s?.[id]||0));
...
function survRunAvail(s){const helperIds=s.helperIds||[],abilityCount=survPsyAbilityCount(s);return SURV_RUN_UPS.filter(u=>{const lv=survRunUpLv(s,u.id),helperMaxReady=!!u.helperKey&&lv>=u.max&&!survRunMaxed(s,u.id)&&(!u.maxReq||survRunRequirementLv(s,u.maxReq)>=5),normalMaxReady=!u.helperKey&&lv>=u.max&&!survRunMaxed(s,u.id)&&['pierce','vortex','rain','icePillars'].includes(u.id),base=lv<u.max||helperMaxReady||normalMaxReady;if(!base)return false;if(u.requiresHelper&&!helperIds.includes(u.requiresHelper))return false;if(u.helperKey&&!helperIds.includes(u.helperKey)&&helperIds.length>=SURV_HELPER_CAP)return false;if(SURV_PSY_ABILITY_IDS.has(u.id)&&!survPsyAbilityActive(s,u.id)&&abilityCount>=SURV_PSY_ABILITY_CAP)return false;return true})}
function survPowerIcon(u,s=null,targetMax=false){
  if(u?.helperKey&&s){const current=survHelperLv(s,u.helperKey),targetLv=targetMax?current:Math.min(u.max,current+1),vis={id:u.poke,shiny:false,mega:false,megaForm:null};if(u.helperKey==='blissey'||u.helperKey==='lapras'){if(targetMax||targetLv>=3)vis.shiny=true}else if(u.helperKey==='wobbuffet'){}else if(targetMax){vis.shiny=true;vis.mega=true}else if(targetLv>=5)vis.mega=true;else if(targetLv>=3)vis.shiny=true;if(vis.mega&&Number(u.poke)===6){s.helperForms=s.helperForms||{};if(!s.helperForms.charizardMega)s.helperForms.charizardMega=Math.random()<.5?'charizard-megax':'charizard-megay';vis.megaForm=s.helperForms.charizardMega}try{return window.getRealSprite?.({id:vis.id,shiny:vis.shiny,isMega:vis.mega,megaForm:vis.megaForm})||`https://raw.githubusercontent.com/PokeAPI/sprites/master/sprites/pokemon/${vis.shiny?'shiny/':''}${vis.id}.png`}catch(_){} }
...
function survApplyUpgrade(s,u){
  ...
  }else if(maxOffer){s.upgradeMaxed[id]=true;if(id==='pierce')s.pierce+=3;if(id==='vortex')s.vortexMax=true;if(id==='rain')s.rainMax=true;if(id==='icePillars'){s.icePillarsMax=true;s.icePillars=5;}}
  else if(lv<u.max){s.upgradeLevels[id]=lv+1;if(id==='rate')s.fireRate=Math.max(135,s.fireRate*.92);if(id==='multi')s.multi=Math.max(1,Number(s.multi||1))+1;if(id==='pierce')s.pierce=Math.max(0,Number(s.pierce||0))+1;if(id==='speed')s.speed*=1.06;if(id==='vortex')s.vortex++;if(id==='rain')s.rain++;if(id==='icePillars')s.icePillars=lv+1;if(id==='regen'){const hpSteps=[0,.03,.05,.07,.10,.15],oldPct=hpSteps[lv]||0,newPct=hpSteps[Math.min(5,lv+1)]||oldPct,oldMax=s.maxHp;s.maxHp*=((1+newPct)/(1+oldPct));s.hp=Math.min(s.maxHp,s.hp+(s.maxHp-oldMax));s.regen+=.0025}}else return false;
  return true
}
...
function survOwnedUpgradeable(s){return SURV_RUN_UPS.filter(u=>{const lv=survRunUpLv(s,u.id);if(lv<=0&&!survRunMaxed(s,u.id))return false;if(survRunMaxed(s,u.id))return false;if(lv<u.max)return true;if(u.helperKey)return lv>=u.max&&(!u.maxReq||survRunRequirementLv(s,u.maxReq)>=5);return lv>=u.max&&['pierce','vortex','rain','icePillars'].includes(u.id)})}
...
function survReflectEnemyProjectile(s,eb,key){if(!eb||eb.dead||eb._reflected)return false;const i=(s.helperIds||[]).indexOf(key);if(i<0)return false;const h=survHelperState(s,key,i);if(!survWobbuffetMirrorActive(s,h))return false;const vx=Number(eb.vx||0),vy=Number(eb.vy||0),sp=Math.max(3,Math.hypot(vx,vy)),mag=Math.max(.001,Math.hypot(vx,vy)),rvx=-vx/mag*sp,rvy=-vy/mag*sp;survBullet(s,{x:eb.x,y:eb.y,vx:rvx,vy:rvy,dmg:Math.max(1,Number(eb.dmg||1)*1.5),pierce:0,helper:true,kind:'wobbuffetReflect',source:'Wobbuffet',color:'#dbeafe',radius:9,life:2400,maxRange:Math.max(Number(s.attackRange||0),SURV_ATTACK_RANGE),homing:false,target:null,reflected:true});eb.dead=true;eb._reflected=true;h.lastReflectAt=Number(s.elapsed||0);survFxPush(s,{type:'mirrorFlash',x:eb.x,y:eb.y,r:12,max:survWobbuffetMirrorRadius(s,key),life:20,color:'#bfdbfe',mirrorCount:survWobbuffetMirrorCount(s,key)});return true}
...
function survNova(s){W.psyAudio?.sfx?.('psy_nova',{volume:.9,cooldown:500});survPsyAnimTrigger(s,'special',500);const pal=survPsyAttackPalette(s),range=Math.min(s.attackRange,135+22*s.nova),dmg=18*s.power*(1+s.nova*.18);survHitArea(s,s.x,s.y,range,dmg,pal.special,0,'Psy Nova');survFxPush(s,{type:'nova',x:s.x,y:s.y,r:20,max:range,life:28,color:pal.special,color2:pal.shot});survFxParticles(s,s.x,s.y,pal.accent,12,4.4,5,25,.02);s.shake=Math.max(Number(s.shake||0),3);if(s.novaMax)setTimeout(()=>{if(W.PSY_CLEAN_SURV===s&&!s.done){survHitArea(s,s.x,s.y,range,12*s.power,pal.accent,0,'Psy Nova');survFxPush(s,{type:'nova',x:s.x,y:s.y,r:16,max:range*.92,life:22,color:pal.shot,color2:pal.special,reverse:true})}},180)}
function survVortex(s){if(!s.vortex)return;W.psyAudio?.sfx?.('psy_vortex',{volume:.82,cooldown:650});survPsyAnimTrigger(s,'special',500);const pal=survPsyAttackPalette(s),range=Math.min(s.attackRange,s.vortexMax?SURV_SQM*10:105+s.vortex*18),dmg=(9+s.vortex*4)*s.power,minDist=SURV_SQM*3,pullStep=52,repelStep=68;
  for(const e of s._liveEnemies||s.enemies){if(e.dead)continue;const dx=s.x-e.x,dy=s.y-e.y,d=Math.hypot(dx,dy)||1;if(d>=range)continue;
    if(s.vortexMax){if(d>minDist){const step=Math.min(pullStep,d-minDist);e.x+=dx/d*step;e.y+=dy/d*step;survDamageEnemy(s,e,dmg,pal.special,0,'Vórtice Mental');if(d-step<=minDist&&!e.dead){e.x-=dx/d*repelStep;e.y-=dy/d*repelStep;survDamageEnemy(s,e,dmg,pal.accent,0,'Vórtice Mental')}}else{e.x-=dx/d*repelStep;e.y-=dy/d*repelStep;survDamageEnemy(s,e,dmg,pal.accent,0,'Vórtice Mental')}}else survDamageEnemy(s,e,dmg,pal.special,0,'Vórtice Mental');
  }
...
function survPsyRain(s){
  ...
  for(const e of live){
    ...
    survDamageEnemy(s,e,dmg,pal.special,0,'Vórtice Mental');survFxImpact(s,e.x,e.y,pal.accent,s.rainMax?42:32,false,1.15);
    if(s.rainMax&&!e.dead){
...
function survLoop(ts){
  ...
  for(const e of s.enemies){
    ...
    if(!frozenNow&&!(e.boss&&e.charge?.active)&&dist<34+e.size*.17){if(tar.kind==='helper')survDamageHelper(s,tar.key,e.hit*dt,e.x,e.y);else{if(survHurtPlayer(s,e.hit*dt)){survBeginDefeat(s);return}}}
  }
