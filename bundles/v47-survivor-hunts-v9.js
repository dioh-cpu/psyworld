...
function survFortunaBonus(s){const lv=Math.max(0,Math.min(5,Number(survRunUpLv(s,'lootGold')||0)));return lv*2+(survRunMaxed(s,'lootGold')?5:0)}
function survSyncFortunaBonus(s){if(!s)return 0;const b=survFortunaBonus(s);s.survLootBonus=Math.max(Number(s.survLootBonus||0),b);s.survGoldBonus=Math.max(Number(s.survGoldBonus||0),b);return b}
function survGoldMultiplier(s){survSyncFortunaBonus(s);return(1+Math.max(0,Number(getTotalBuff?.('gold')||0))/100)*(1+Math.max(0,Number(s?.survGoldBonus||0))/100)}
const SURV_RUN_UPS=[
...
function survMigrateRemovedSpeedUpgrade(run){
  ...
  run.survGoldBonus=Math.max(0,Number(run.survGoldBonus||0));survSyncFortunaBonus(run);
}
