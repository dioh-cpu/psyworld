...
(function(W,D){
  ...
  function hookEconomy(){
    const world=W.handleWorldKill;if(typeof world==='function'&&!world.__psyGlobalWrapped){W.handleWorldKill=function(mon){const out=world.apply(this,arguments);cardDrop(mon,'world');return out};W.handleWorldKill.__psyGlobalWrapped=true}
    const action=W.battleAction;if(typeof action==='function'&&!action.__psyGlobalWrapped){W.battleAction=function(){const out=action.apply(this,arguments);try{const bd=W.battleData;if(bd?.state==='won'&&bd.wild)cardDrop(bd.wild,'battle')}catch(e){}return out};W.battleAction.__psyGlobalWrapped=true}
