...
function enemyCounter(){
  ...
  try{
    ...
    let atk=Number(bd?.wildAtk||w?.atk||0);
    if(!atk){try{atk=Number(W.psyBattleWildBaseAtkV27?.(w)||0)}catch(_){}}
    if(!atk)atk=Math.max(12,Math.floor(Number(w.level||w.lvl||5)*3.5+Number(bd.wildMaxHp||100)*.025));
    const dmg=Math.max(1,Math.floor(atk*.45*(100/(100+Math.max(0,def)*.15))));
