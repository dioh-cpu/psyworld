...
const BUILD='CAPTURE_V40_QUALITY_EVOLUTION_BONUS_20260906';
const FINAL_CAP=30;
...
function captureBonusPoints(){
  ...
  let n=Math.max(0,Number(P0.worldBuffs?.cap||0))+Math.max(0,Number(P0.trainerBuffs?.cap||0));
...
W.psyCaptureBreakdownV40=function(ballName,wildArg){return captureDetails(ballName,wildArg)};
...
function install(){
  ...
  W.tryCaptureBattle=fixed;try{tryCaptureBattle=fixed}catch(_){}console.log('🎯 PSYWORLD Capture V40 instalado • qualidade/evolução/força • bônus aditivo • teto 30%');
}
...
console.log('🎯 PSYWORLD Capture V40 carregado',BUILD);
})(window,document);
